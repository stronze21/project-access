<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl-removed mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Narrow categories by keyword and status.</p>
                    </div>
                    @if ($hasActiveFilters)
                ...
                <form method="GET" action="{{ route('complaints.categories.index') }}" class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                    <div class="sm:col-span-2">
                        ...
                        <input id="q"
                               ...
                               class="mt-1 block w-full rounded-lg border-slate-300 text-sm placeholder:text-slate-400 focus:border-rose-500 focus:ring-rose-500">
                    </div>
                    <div>
                        <label for="state" class="text-xs font-semibold uppercase tracking-wide text-slate-500">Status</label>
                        <select id="state" name="state" class="mt-1 block w-full rounded-lg border-slate-300 text-sm focus:border-rose-500 focus:ring-rose-500">
                    ...
                    <div class="sm:col-span-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
                        <button type="submit"
                                class="inline-flex items-center justify-center rounded-lg bg-rose-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-rose-700">
                            Apply Filters
                        </button>
                    </div>
                </form>
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                <div class="mb-4">
                    <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-800">Create Category</h3>
                    <p class="text-xs text-slate-500">Add a new complaint category for citizen submissions and internal routing.</p>
                </div>
                <form method="POST" action="{{ route('complaints.categories.store') }}" class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <input id="new_name"
                               ...
                               class="mt-1 block w-full rounded-lg border-slate-300 text-sm focus:border-rose-500 focus:ring-rose-500"
                               required>
                    </div>
                    <div class="lg:col-span-2">
                    ...
                    <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-slate-500">{{ $category->complaints_count }} linked complaints</p>
                            </div>
                        ...
                        <form method="POST" action="{{ route('complaints.categories.update', $category) }}" class="mt-4 space-y-2">
                            ...
                            <input type="text"
                                   name="name"
                                   value="{{ $category->name }}"
                                   class="block w-full rounded-lg border-slate-300 text-sm focus:border-rose-500 focus:ring-rose-500"
                                   required>
                            <input type="text"
                                   name="description"
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Linked Cases</th>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($categories as $category)
                                ...
                                <tr class="hover:bg-slate-50/70">
                                    <td class="px-4 py-3 text-sm">
                                        ...
                                        <p class="text-xs text-slate-500">{{ $category->description ?: 'No description provided.' }}</p>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-slate-700">{{ number_format((int) $category->complaints_count) }}</td>
                                    <td class="px-4 py-3 text-sm">
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <form method="POST" action="{{ route('complaints.categories.update', $category) }}" class="grid grid-cols-1 gap-2">
                                            ...
                                            <input type="text"
                                                   name="name"
                                                   value="{{ $category->name }}"
                                                   class="block w-full rounded-lg border-slate-300 text-xs focus:border-rose-500 focus:ring-rose-500"
                                                   required>
                                            <input type="text"
                                                   name="description"
                                                   value="{{ $category->description }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-10 text-center text-sm text-slate-600">No categories found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
