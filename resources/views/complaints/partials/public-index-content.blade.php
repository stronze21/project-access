...
<div class="space-y-6">
    <section class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-blue-900 to-cyan-700 p-5 shadow-lg sm:p-7">
            ...
            <div class="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
                @if ($canSubmitAnonymous)
                    <a href="{{ route('complaints.anonymous.create') }}"
                       class="inline-flex items-center justify-center rounded-lg border border-white/30 bg-white/10 px-3 py-2 text-sm font-semibold text-white backdrop-blur hover:bg-white/20">
                        Submit Anonymous
                    </a>
                @endif
                @auth
                    @if (auth()->user()->isCitizen())
                        <a href="{{ route('complaints.create') }}"
                           class="inline-flex items-center justify-center rounded-lg bg-white px-3 py-2 text-sm font-semibold text-blue-800 hover:bg-blue-50">
                            Submit Verified
                        </a>
                    @endif
                @endauth
        ...
        <div class="relative mt-5 grid grid-cols-2 gap-2 sm:max-w-md">
            <div class="rounded-lg border border-white/20 bg-white/10 px-3 py-2 backdrop-blur">
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-800">Filters</h2>
                <p class="text-xs text-slate-500">Refine by status and category.</p>
            </div>
            @if ($hasActiveFilters)
        ...
        <form method="GET" class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <div class="lg:col-span-2">
                <label for="category_id" class="text-xs font-semibold uppercase tracking-wide text-slate-500">Category</label>
                <select id="category_id" name="category_id" class="mt-1 block w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
    ...
    <section class="space-y-4">
        @forelse ($complaints as $complaint)
            ...
            <article class="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:shadow-md">
                ...
                <div class="p-4 sm:p-5">
                        ...
                        <a href="{{ route('complaints.preview-image', $complaint) }}"
                           ...
                           class="block">
                            <img src="{{ route('complaints.preview-image', $complaint) }}"
                                 alt="Complaint photo"
                                 class="mb-3 h-44 w-full cursor-zoom-in rounded-xl border border-slate-200 object-cover sm:h-52">
                        </a>
                        <p class="-mt-1 mb-2 text-[11px] text-slate-500">Tap image to enlarge</p>
                    @endif

                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div class="min-w-0">
                            <h2 class="truncate text-base font-semibold text-slate-900 sm:text-lg">{{ $complaint->title }}</h2>
                            <p class="mt-1 text-xs text-slate-500">Reference: {{ $complaint->reference_code }}</p>
                            ...
                            @elseif ($complaint->visibility === \App\Models\Complaint::VISIBILITY_PUBLIC_ANONYMOUS)
                                <p class="mt-2 text-xs text-slate-500">Reported anonymously</p>
                            @endif
                        </div>
                    ...
                    <p class="mt-3 text-sm leading-relaxed text-slate-700">{{ $complaint->short_summary }}</p>

                    <dl class="mt-4 grid grid-cols-1 gap-2 text-sm text-slate-600 sm:grid-cols-3">
                        <div class="rounded-lg bg-slate-50 px-3 py-2">
                            <dt class="text-xs font-semibold uppercase tracking-wide text-slate-500">Category</dt>
                            <dd class="mt-0.5 font-medium text-slate-700">{{ $complaint->category?->name ?? 'N/A' }}</dd>
                        </div>
                        <div class="rounded-lg bg-slate-50 px-3 py-2">
                            <dt class="text-xs font-semibold uppercase tracking-wide text-slate-500">Barangay</dt>
                            <dd class="mt-0.5 font-medium text-slate-700">{{ $complaint->barangay?->name ?? 'Not specified' }}</dd>
                        </div>
                        <div class="rounded-lg bg-slate-50 px-3 py-2">
                            <dt class="text-xs font-semibold uppercase tracking-wide text-slate-500">Supports</dt>
                            <dd class="mt-0.5 font-medium text-slate-700">{{ number_format((int) $complaint->support_count) }}</dd>
                        </div>
                    </dl>
                    ...
                    <div class="mt-4 space-y-1 text-xs text-slate-500">
                        ...
                        <p>{{ $complaint->timeMetricTitle() }}: <span class="font-semibold text-slate-700">{{ $complaint->runningTimeLabel() }}</span></p>
                    </div>
                    ...
                    <div class="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3">
                        ...
                        <div class="mt-3 space-y-2">
                            @forelse ($complaint->visibleComments as $comment)
                                ...
                                @endphp
                                <article id="comment-{{ $comment->id }}" class="rounded-lg border border-slate-200 bg-white p-2.5">
                                    <div class="flex items-center justify-between gap-3">
                                        ...
                                        <p class="text-[11px] text-slate-500">{{ $comment->createdAtManila()?->format('M d, Y h:i A') }}</p>
                                    </div>
                                    <p class="mt-1.5 text-sm text-slate-700">{{ $comment->body }}</p>
                                    ...
                                    <div class="mt-2 flex flex-wrap items-center gap-1.5">
                                        @auth
                                            <form method="POST" action="{{ route('complaints.comments.react', [$complaint, $comment]) }}">
                                                ...
                                                <button type="submit"
                                                        ...
                                                        class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-semibold {{ $currentReaction === 'like' ? 'bg-emerald-100 text-emerald-700 ring-1 ring-emerald-300' : 'bg-slate-100 text-slate-700 hover:bg-slate-200' }}">
                                            ...
                                            <form method="POST" action="{{ route('complaints.comments.react', [$complaint, $comment]) }}">
                                                ...
                                                <button type="submit"
                                                        ...
                                                        class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-semibold {{ $currentReaction === 'dislike' ? 'bg-rose-100 text-rose-700 ring-1 ring-rose-300' : 'bg-slate-100 text-slate-700 hover:bg-slate-200' }}">
                                                    <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                        ...
                                        @else
                                            <span class="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-1 text-[11px] font-semibold text-slate-700">
                                                <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                                    <path d="M7 10v12"></path>
                                                    <path d="M15 5.88 14 10h5.83a2 2 0 0 1 1.92 2.56l-2.33 8A2 2 0 0 1 17.5 22H4a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2.76a2 2 0 0 0 1.79-1.11L12 2a3.13 3.13 0 0 1 3 3.88Z"></path>
                                                </svg>
                    ...
                    <div class="mt-3 flex items-center justify-end">
                        <a href="{{ route('complaints.public.show', $complaint) }}"
                           class="inline-flex items-center gap-1 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-700 transition hover:bg-blue-100">
                            View Details
        ...
        @empty
            <div class="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center shadow-sm">
                <p class="text-base font-semibold text-slate-800">No public complaints found</p>
                <p class="mt-1 text-sm text-slate-500">Try changing filters or submit a new complaint.</p>
                <div class="mt-4 flex flex-wrap items-center justify-center gap-2">
                    <a href="{{ route('complaints.public.index') }}"
                       class="inline-flex rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                        Reset Filters
                    </a>
                    @if ($canSubmitAnonymous)
                        <a href="{{ route('complaints.anonymous.create') }}"
