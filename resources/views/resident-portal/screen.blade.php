...
@php
    ...
    $statusClass = fn (?string $status) => in_array($status, ['completed','released','distributed','resolved','closed','active'], true) ? 'success' : (in_array($status, ['rejected','cancelled','urgent','critical'], true) ? 'danger' : 'warning');
    $photoUrl = null;
...
@elseif($screen === 'settings')
    ...
    <form class="native-card form-stack" method="POST" action="{{ route('resident-portal.profile.photo') }}" enctype="multipart/form-data">@csrf<label>Choose a photo<input type="file" name="photo" accept="image/jpeg,image/png" capture="user" required></label><button class="primary-button" type="submit">Update photo</button></form>
    ...
    <p class="section-header">Signature</p><form class="native-card form-stack" method="POST" action="{{ route('resident-portal.profile.signature') }}" data-signature-form>@csrf<canvas class="signature-pad" width="400" height="170" data-signature-pad aria-label="Draw your signature"></canvas><input type="hidden" name="signature" data-signature-value><div class="signature-actions"><button class="outline-button" type="button" data-signature-clear>Clear</button><button class="primary-button" type="submit">Save signature</button></div></form>
