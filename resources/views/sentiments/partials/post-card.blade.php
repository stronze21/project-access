...
<article id="post-{{ $post->id }}"
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div class="min-w-0">
            <div class="flex items-center gap-2">
                ...
                <div class="min-w-0">
                    <p class="truncate text-sm font-semibold text-slate-900">{{ $post->author?->name ?? 'Deleted user' }}</p>
        ...
        <div class="flex flex-wrap items-center gap-1.5">
            @if ($post->is_pinned)
                <span class="rounded-full bg-cyan-100 px-2 py-1 text-[11px] font-semibold text-cyan-700">Pinned</span>
            @endif
            @if ($post->is_comments_locked)
                <span class="rounded-full bg-amber-100 px-2 py-1 text-[11px] font-semibold text-amber-700">Comments Locked</span>
            @endif
            @if ($post->hidden_at && ($isOwnPost || $canModerate))
                <span class="rounded-full bg-amber-100 px-2 py-1 text-[11px] font-semibold text-amber-700">Hidden</span>
            @endif
        </div>
    </div>
    ...
    <div class="mt-3 space-y-3">
        @if ($isSoftDeletedPost)
            <p class="rounded-lg bg-slate-50 px-3 py-3 text-sm italic text-slate-500">This post was deleted.</p>
        @elseif ($isPermanentDeletedPost)
            <p class="rounded-lg bg-slate-50 px-3 py-3 text-sm italic text-slate-500">This post was permanently removed by moderation.</p>
        @else
            @if ($post->body)
                <p class="whitespace-pre-line text-sm leading-relaxed text-slate-700">{{ $post->body }}</p>
            @endif

            @if ($post->media_kind === \App\Models\SentimentPost::MEDIA_IMAGE)
                <img src="{{ route('sentiments.posts.media', $post) }}"
                     alt="Post image"
                     class="max-h-[28rem] w-full rounded-xl border border-slate-200 object-cover">
            @elseif ($post->media_kind === \App\Models\SentimentPost::MEDIA_VIDEO)
                <video controls class="max-h-[28rem] w-full rounded-xl border border-slate-200 bg-black">
    ...
    <div class="mt-4 flex flex-wrap items-center gap-1.5 border-t border-slate-100 pt-3">
                    ...
                    @foreach ($reactionTypes as $reactionKey => $reactionMeta)
                        <form method="POST" action="{{ route('sentiments.posts.react', $post) }}">
                            ...
                            <button type="submit"
                                    ...
                                    class="inline-flex h-9 w-9 items-center justify-center rounded-full text-xl transition hover:-translate-y-1 hover:scale-125 focus:outline-none focus:ring-2 focus:ring-blue-400 {{ $currentPostReaction === $reactionKey ? 'bg-blue-50' : '' }}">
        ...
        <button type="button"
                ...
                title="Comments"
                class="inline-flex h-8 items-center gap-1 rounded-full bg-slate-100 px-3 text-[11px] font-semibold text-slate-700 transition hover:bg-slate-200">
            <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
    ...
    <div class="mt-3 flex flex-wrap items-center gap-1.5">
            ...
            @else
                <form method="POST" action="{{ route('sentiments.users.follow', $post->author) }}">
                    ...
                    <button type="submit"
                            title="Follow"
                            aria-label="Follow"
                            class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-cyan-100 text-cyan-700 hover:bg-cyan-200">
                        <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        @if ($isOwnPost && !$isSoftDeletedPost && !$isPermanentDeletedPost)
            <details>
                <summary class="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-slate-100 text-slate-700 hover:bg-slate-200"
                         title="Edit post"
                ...
                <form method="POST" action="{{ route('sentiments.posts.update', $post) }}" class="mt-2 space-y-2">
                    ...
                    <textarea name="body"
                              ...
                              required
                              maxlength="5000"
                              class="w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">{{ $post->body }}</textarea>
            ...
            <form method="POST" action="{{ route('sentiments.posts.destroy', $post) }}"
                ...
                <button type="submit"
                        ...
                        class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-rose-100 text-rose-700 hover:bg-rose-200">
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        @if (!$isOwnPost && !$isSoftDeletedPost && !$isPermanentDeletedPost)
            <form method="POST" action="{{ route('sentiments.posts.report', $post) }}">
                ...
                <button type="submit"
                        ...
                        class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-amber-100 text-amber-700 hover:bg-amber-200">
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        @if ($canModerate && !$isSoftDeletedPost)
            ...
            <form method="POST" action="{{ route('sentiments.posts.moderate', $post) }}">
                ...
                <button type="submit" class="rounded-md bg-cyan-100 px-2 py-1 text-[11px] font-semibold text-cyan-700 hover:bg-cyan-200">
    ...
    @if (!$isSoftDeletedPost && !$isPermanentDeletedPost)
        <div x-show="!commentsOpen" class="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3">
            <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Most Recent Comment</p>
            @if ($latestComment)
                <div class="mt-2 flex items-start gap-2">
                    <x-user-avatar :user="$latestComment->author" size="h-7 w-7" textSize="text-[10px]" />
                    <div class="min-w-0">
                        <p class="truncate text-xs font-semibold text-slate-700">{{ $latestComment->author?->name ?? 'Deleted user' }}</p>
                        <p class="mt-1 text-sm text-slate-600">{{ $latestCommentPreview }}</p>
                    </div>
                </div>
            @else
                <p class="mt-2 text-sm text-slate-500">No comments yet. Tap the message icon to start the discussion.</p>
            @endif
        </div>
    @endif
    ...
    <div x-show="commentsOpen"
        ...
        <p class="text-xs font-semibold uppercase tracking-wide text-slate-600">Comments</p>
