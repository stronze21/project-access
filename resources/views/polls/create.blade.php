    ...
    <div class="py-6">
        <div class="mx-auto max-w-6xl space-y-5 px-4 sm:px-6 lg:px-8">
            <section class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-indigo-900 to-cyan-700 p-5 text-white shadow-lg sm:p-6">
                ...
                <div class="relative flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    ...
                    <div class="grid grid-cols-2 gap-2 text-xs">
                        <span class="rounded-full border border-white/25 bg-white/10 px-2.5 py-1 font-semibold backdrop-blur">2-10 options</span>
                        <span class="rounded-full border border-white/25 bg-white/10 px-2.5 py-1 font-semibold backdrop-blur">One vote per user</span>
                    </div>
                </div>
            </section>
            ...
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
                <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 lg:col-span-2">
                    <form method="POST" action="{{ route('polls.store') }}" class="space-y-5">
                        ...
                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <h4 class="mt-1 text-sm font-semibold text-slate-800">Write the poll prompt</h4>

                            <div class="mt-3">
                                <label for="question" class="text-xs font-semibold uppercase tracking-wide text-slate-500">Question</label>
                                <input id="question"
                                       name="question"
                                       type="text"
                                       value="{{ old('question') }}"
                                       placeholder="What project should be prioritized next quarter?"
                                       class="mt-1 block w-full rounded-lg border-slate-300 text-sm focus:border-cyan-600 focus:ring-cyan-600"
                                       required>
                            </div>

                            <div class="mt-3">
                                <label for="description" class="text-xs font-semibold uppercase tracking-wide text-slate-500">Description (Optional)</label>
                                <textarea id="description"
                                          name="description"
                                          rows="3"
                                          placeholder="Add context so users understand what they are voting for."
                                          class="mt-1 block w-full rounded-lg border-slate-300 text-sm focus:border-cyan-600 focus:ring-cyan-600">{{ old('description') }}</textarea>
                            </div>
                        </div>

                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Step 2</p>
                            <h4 class="mt-1 text-sm font-semibold text-slate-800">Add response options</h4>

                            <div class="mt-3">
                                <label for="options_text" class="text-xs font-semibold uppercase tracking-wide text-slate-500">Options (One Per Line)</label>
                                ...
                                <p class="mt-1 text-xs text-slate-500">Minimum 2 options, maximum 10 options. Keep choices short and distinct.</p>
                            </div>
                        </div>

                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Step 3</p>
                            <h4 class="mt-1 text-sm font-semibold text-slate-800">Set poll availability</h4>

                            <div class="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                                <div>
                                    <label for="ends_at" class="text-xs font-semibold uppercase tracking-wide text-slate-500">End Date/Time (Optional)</label>
                                    <input id="ends_at"
                                           name="ends_at"
                                           type="datetime-local"
                                           value="{{ old('ends_at') }}"
                                ...
                                <div class="flex items-end">
                                    <label class="inline-flex w-full items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
                ...
                <aside class="space-y-4">
                    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                        <h4 class="text-sm font-semibold uppercase tracking-wide text-slate-700">Publishing Checklist</h4>
                        <div class="mt-3 space-y-2 text-sm text-slate-600">
                            <p class="rounded-lg bg-slate-50 px-3 py-2">1. Ask one clear question.</p>
                            <p class="rounded-lg bg-slate-50 px-3 py-2">2. Add at least 2 options.</p>
                            <p class="rounded-lg bg-slate-50 px-3 py-2">3. Avoid overlapping options.</p>
                            <p class="rounded-lg bg-slate-50 px-3 py-2">4. Set an end time for time-bound polls.</p>
                        </div>
                    </section>
