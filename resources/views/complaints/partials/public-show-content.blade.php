...
<div class="space-y-5">
        ...
        <div class="relative">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-xs text-white/75">Reference: {{ $complaint->reference_code }}</p>
                </div>
            ...
            <p class="mt-4 max-w-3xl text-sm text-white/80">{{ $complaint->short_summary }}</p>
        </div>
    </article>

    @if ($complaint->previewImageAttachment)
        <section class="rounded-2xl border border-base-300 bg-base-100 p-3 shadow-sm sm:p-4 card">
            ...
            <p class="mt-2 text-xs text-base-content/60">Tap image to enlarge</p>
        </section>
    @endif
    ...
    <section class="grid grid-cols-2 gap-3 lg:grid-cols-6">
        <div class="rounded-xl bg-base-100 p-3 shadow-sm ring-1 ring-base-300 sm:p-4">
            ...
            <p class="mt-1 text-sm font-bold text-base-content">{{ $complaint->category?->name ?? 'N/A' }}</p>
        </div>
        <div class="rounded-xl bg-base-100 p-3 shadow-sm ring-1 ring-base-300 sm:p-4">
            <p class="text-[11px] font-semibold uppercase tracking-wide text-base-content/60">Barangay</p>
            <p class="mt-1 text-sm font-bold text-base-content">{{ $complaint->barangay?->name ?? 'Not specified' }}</p>
        </div>
        <div class="rounded-xl bg-base-100 p-3 shadow-sm ring-1 ring-base-300 sm:p-4">
            <p class="text-[11px] font-semibold uppercase tracking-wide text-base-content/60">Support Count</p>
            <p class="mt-1 text-sm font-bold text-base-content">{{ number_format((int) $complaint->support_count) }}</p>
        </div>
        <div class="rounded-xl bg-base-100 p-3 shadow-sm ring-1 ring-base-300 sm:p-4">
    ...
    <section class="rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm sm:p-6 card">
        ...
        @if ($complaint->officials->isNotEmpty())
            <div class="mt-4">
                <p class="text-xs font-semibold uppercase tracking-wide text-base-content/60">Tagged Public Officials</p>
        ...
        @if ($complaint->resolution_summary)
            <div class="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 alert alert-success">
            ...
            @auth
                @can('support', $complaint)
                    <form method="POST" action="{{ route('complaints.support', $complaint) }}">
                        @csrf
                        <button type="submit" class="inline-flex rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white hover:bg-blue-700 btn btn-primary btn-sm">
                            I Have the Same Issue
                        </button>
                    </form>
                @endcan
            @endauth
            <a href="{{ route('complaints.public.index') }}" class="inline-flex rounded-lg border border-base-300 px-3 py-2 text-sm font-semibold text-base-content/80 hover:bg-base-200 btn btn-outline btn-sm">
                Back to List
            </a>
        </div>
    </section>
    ...
    <section class="rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm sm:p-6 card">
        <h2 class="text-base font-semibold text-base-content">Public Comments</h2>
        ...
        <div class="mt-4 space-y-3">
            @forelse ($complaint->visibleComments as $comment)
                ...
                @endphp
                <article id="comment-{{ $comment->id }}" class="rounded-xl border border-base-300 p-3">
                    ...
                    <p class="mt-2 text-sm text-base-content/80">{{ $comment->body }}</p>
                    ...
                    <div class="mt-3 flex flex-wrap items-center gap-2">
                        @auth
                            <form method="POST" action="{{ route('complaints.comments.react', [$complaint, $comment]) }}">
                                ...
                                <button type="submit"
                                        ...
                                        class="inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-semibold {{ $currentReaction === 'like' ? 'bg-emerald-100 text-emerald-700 ring-1 ring-emerald-300' : 'bg-base-200 text-base-content/80 hover:bg-base-300' }} btn btn-ghost btn-xs">
                            ...
                            <form method="POST" action="{{ route('complaints.comments.react', [$complaint, $comment]) }}">
                                ...
                                <button type="submit"
                                        ...
                                        class="inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-semibold {{ $currentReaction === 'dislike' ? 'bg-rose-100 text-rose-700 ring-1 ring-rose-300' : 'bg-base-200 text-base-content/80 hover:bg-base-300' }} btn btn-ghost btn-xs">
                                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                        ...
                        @else
                            <span class="inline-flex items-center gap-1.5 rounded-md bg-base-200 px-2.5 py-1 text-xs font-semibold text-base-content/80">
                                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    <path d="M7 10v12"></path>
                                    <path d="M15 5.88 14 10h5.83a2 2 0 0 1 1.92 2.56l-2.33 8A2 2 0 0 1 17.5 22H4a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2.76a2 2 0 0 0 1.79-1.11L12 2a3.13 3.13 0 0 1 3 3.88Z"></path>
                                </svg>
            ...
            @empty
                <p class="text-sm text-base-content/70">No public comments yet.</p>
            @endforelse
        </div>
        ...
        @auth
            <form method="POST" action="{{ route('complaints.comments.store', $complaint) }}" class="mt-4 space-y-2">
                @csrf
                <label for="body" class="text-sm font-semibold text-base-content/80">Add Comment</label>
                <textarea id="body" name="body" rows="3" class="block w-full rounded-lg border-base-300 text-sm focus:border-blue-500 focus:ring-blue-500 textarea textarea-bordered" required>{{ old('body') }}</textarea>
                @error('body')
