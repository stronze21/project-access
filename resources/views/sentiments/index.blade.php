<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">People Sentiments</h2>
                <p class="text-xs text-slate-500">Global Facebook-like feed for all signed-in users.</p>
            </div>
                ...
                <div class="xl:col-span-2 space-y-4">
                    <div class="flex flex-wrap items-center gap-2">
                        <a href="{{ route('sentiments.index', request()->except(['page'])) }}"
                           class="inline-flex rounded-lg px-3 py-2 text-sm font-semibold {{ $activeMode === 'feed' ? 'bg-blue-600 text-white' : 'bg-white text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50' }}">
                            Global Feed
                        </a>
                        <a href="{{ route('sentiments.trending', request()->except(['page'])) }}"
                           class="inline-flex rounded-lg px-3 py-2 text-sm font-semibold {{ $activeMode === 'trending' ? 'bg-blue-600 text-white' : 'bg-white text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50' }}">
                            Trending
                        </a>
                    </div>
                    ...
                    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                        <div class="flex items-center justify-between gap-3">
                            <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-800">Create Post</h3>
                            <span class="text-[11px] text-slate-500">Max 5,000 characters · Real identity only</span>
                        </div>
                        ...
                        @else
                            <form method="POST" action="{{ route('sentiments.posts.store') }}" enctype="multipart/form-data" class="mt-3 space-y-3">
                                ...
                                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                                    <div>
                                        <label for="media" class="text-xs font-semibold uppercase tracking-wide text-slate-500">Upload Photo/Video (Optional)</label>
                                        <input id="media"
                                               type="file"
                                               name="media"
                                               accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/quicktime"
                                               class="mt-1 block w-full rounded-lg border border-slate-300 text-sm file:mr-3 file:rounded-md file:border-0 file:bg-slate-100 file:px-3 file:py-2 file:text-xs file:font-semibold file:text-slate-700 hover:file:bg-slate-200">
                                    </div>
                                    <div>
                                        <label for="external_url" class="text-xs font-semibold uppercase tracking-wide text-slate-500">External Media URL (Optional)</label>
                                ...
                                <button type="submit" class="inline-flex rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">
                                    Publish Post
                                </button>
                            </form>
                        @endif
                        ...
                        <details class="group">
                            <summary class="flex cursor-pointer list-none items-center justify-between gap-2 rounded-lg [&::-webkit-details-marker]:hidden">
                                <div class="flex items-center gap-2">
                                    <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-800">Advanced Search</h3>
                                    @if ($hasAdvancedFilters)
                                        <span class="rounded-full bg-blue-100 px-2 py-0.5 text-[11px] font-semibold text-blue-700">Filters Active</span>
                                    @endif
                                </div>
                                <span class="inline-flex rounded-md bg-slate-100 px-2 py-1 text-[11px] font-semibold text-slate-600">Expand / Collapse</span>
                            </summary>
                            ...
                            <form method="GET" class="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6">
                                <div class="xl:col-span-2">
                                    <label for="q" class="text-xs font-semibold uppercase tracking-wide text-slate-500">Keyword</label>
                                    <input id="q"
                                           type="text"
                                           name="q"
                                           value="{{ request('q') }}"
                                ...
                                <div>
                                    <label for="author_id" class="text-xs font-semibold uppercase tracking-wide text-slate-500">Author</label>
                                    <select id="author_id" name="author_id" class="mt-1 w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                <div>
                                    <label for="reaction_type" class="text-xs font-semibold uppercase tracking-wide text-slate-500">Reaction</label>
                                    <select id="reaction_type" name="reaction_type" class="mt-1 w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                <div>
                                    ...
                                    <input id="date_from" type="date" name="date_from" value="{{ request('date_from') }}" class="mt-1 w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                                </div>
                                <div>
                                    <label for="date_to" class="text-xs font-semibold uppercase tracking-wide text-slate-500">Date To</label>
                                    <input id="date_to" type="date" name="date_to" value="{{ request('date_to') }}" class="mt-1 w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                                </div>
                                <div>
                                    <label for="comment_sort" class="text-xs font-semibold uppercase tracking-wide text-slate-500">Comments Sort</label>
                                    <select id="comment_sort" name="comment_sort" class="mt-1 w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                <div class="md:col-span-2 xl:col-span-6">
                                    <button type="submit" class="inline-flex rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700">
                                        Apply Filters
                                    </button>
                                </div>
                            </form>
                ...
                <aside class="space-y-4">
                    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                        <div class="flex items-center justify-between">
                            <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-800">In-App Notifications</h3>
                        ...
                        <div class="mt-3 space-y-2">
                            @forelse ($notifications as $notification)
                                ...
                                <article class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                                    <p class="text-xs font-semibold text-slate-800">{{ $data['title'] ?? 'Notification' }}</p>
                                    <p class="mt-0.5 text-xs text-slate-600">{{ $data['message'] ?? 'You have an update.' }}</p>
                    ...
                    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                        <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-800">Rules Snapshot</h3>
    ...
    <div id="sentiment-reactor-modal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute left-1/2 top-1/2 w-[92vw] max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-4 shadow-xl sm:p-5">
            <div class="flex items-center justify-between">
                <h3 class="text-base font-semibold text-slate-900">Reactions</h3>
                <button type="button" class="rounded-md bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-200" data-reactor-close>
                    Close
                </button>
            </div>
            <div id="sentiment-reactor-content" class="mt-3 max-h-[60vh] space-y-2 overflow-y-auto text-sm text-slate-700"></div>
        </div>
    </div>
        ...
        (() => {
            ...
            document.addEventListener('click', async (event) => {
                ...
                try {
                    ...
                    modalContent.innerHTML = '<p class="text-xs text-slate-500">Loading reactions...</p>';
                    ...
                    Object.keys(labels).forEach((key) => {
                        ...
                        rows.push(
                            `<div class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                                <p class="text-xs font-semibold text-slate-800">${emoji} ${label} (${names.length})</p>
                                <p class="mt-1 text-xs text-slate-600">${names.join(', ')}</p>
                            </div>`
                        );
                    });
                    ...
                    modalContent.innerHTML = rows.length
                        ? rows.join('')
                        : '<p class="text-xs text-slate-500">No reactions yet.</p>';
                } catch (error) {
                    modalContent.innerHTML = '<p class="text-xs text-rose-600">Failed to load reactions.</p>';
                }
            });
