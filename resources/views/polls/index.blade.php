<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="font-semibold text-xl text-base-content/90 leading-tight">Community Polls</h2>
            @if ($canManagePolls)
            ...
            <section class="space-y-4">
                @forelse ($polls as $poll)
                    ...
                    <article class="overflow-hidden rounded-2xl border border-base-300 bg-base-100 shadow-sm card">
                        <div class="border-b border-base-300 bg-base-200/80 px-4 py-3 sm:px-5">
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    <h3 class="text-base font-semibold text-base-content sm:text-lg">{{ $poll->question }}</h3>
                                ...
                                <div class="flex flex-wrap gap-2 text-xs">
                                    ...
                                    <span class="rounded-full bg-blue-100 px-2.5 py-1 font-semibold text-blue-700 badge badge-sm">
                                        {{ number_format($totalVotes) }} Votes
                                    </span>
                                </div>
                            </div>
                            @if ($poll->description)
                                <p class="mt-2 text-sm text-base-content/80">{{ $poll->description }}</p>
                            @endif
                            @if ($poll->ends_at)
                                <p class="mt-1 text-xs text-base-content/60">Ends: {{ $poll->ends_at->format('M d, Y h:i A') }}</p>
                            @endif
                        </div>
                        ...
                        <div class="space-y-3 p-4 sm:p-5">
                            @foreach ($poll->options as $option)
                                ...
                                @endphp
                                <div class="rounded-xl border {{ $isSelected ? 'border-blue-300 bg-blue-50/40' : 'border-base-300 bg-base-100' }} p-3">
                                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                        <div class="min-w-0">
                                            <p class="text-sm font-semibold text-base-content">{{ $option->option_text }}</p>
                                        ...
                                        <div class="flex items-center gap-2">
                                            @if ($isOpen)
                                                <form method="POST" action="{{ route('polls.vote', $poll) }}">
                                                    ...
                                                    <button type="submit"
                                                            class="inline-flex rounded-lg px-3 py-1.5 text-xs font-semibold {{ $isSelected ? 'border border-blue-300 text-blue-700 hover:bg-blue-100' : 'bg-blue-600 text-white hover:bg-blue-700' }} btn btn-ghost btn-xs">
                                                        {{ $isSelected ? 'Selected' : 'Vote' }}
                                                    </button>
                                                </form>
                                            @else
                                                <span class="inline-flex rounded-lg border border-base-300 px-3 py-1.5 text-xs font-semibold text-base-content/70">
                                                    Voting Closed
                                                </span>
                                            @endif
                                        </div>
