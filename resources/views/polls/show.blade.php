<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="font-semibold text-xl text-base-content/90 leading-tight">Poll Details</h2>
    ...
    <div class="py-6">
        <div class="mx-auto max-w-5xl space-y-5 px-4 sm:px-6 lg:px-8">
            ...
            <section class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-cyan-900 to-teal-700 p-5 text-white shadow-lg sm:p-6">
                ...
                <div class="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div class="flex flex-wrap gap-2 text-xs">
                        <span class="rounded-full border border-white/30 bg-base-100/10 px-2.5 py-1 font-semibold backdrop-blur badge badge-sm">
                ...
                <div class="relative mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                    <div class="rounded-xl border border-white/20 bg-base-100/10 px-3 py-2 backdrop-blur">
            ...
            <article class="overflow-hidden rounded-2xl border border-base-300 bg-base-100 shadow-sm card">
                <div class="border-b border-base-300 bg-base-200/70 px-4 py-3 sm:px-6">
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <h3 class="text-sm font-semibold uppercase tracking-wide text-base-content/80">Options and Live Results</h3>
                            <p class="text-xs text-base-content/60">Tap <span class="font-semibold">Vote</span> to submit or change your answer while voting is open.</p>
                        </div>
                        <div class="flex flex-wrap gap-2 text-xs">
                            <span class="rounded-full px-2.5 py-1 font-semibold {{ $isOpen ? 'bg-emerald-100 text-emerald-700' : 'bg-base-300 text-base-content/80' }} badge badge-sm">
                                {{ $isOpen ? 'Open' : 'Closed' }}
                            </span>
                            @if ($selectedOption)
                                <span class="rounded-full bg-blue-100 px-2.5 py-1 font-semibold text-blue-700 badge badge-sm">You voted</span>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-3 p-4 sm:p-6">
                    @foreach ($poll->options as $option)
                        ...
                        <div class="rounded-xl border p-3 sm:p-4 {{ $isSelected ? 'border-blue-300 bg-blue-50/40 ring-1 ring-blue-200' : 'border-base-300 bg-base-100' }}">
                            <div class="flex flex-col gap-3">
                                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                    <div class="min-w-0">
                                        <p class="text-sm font-semibold text-base-content sm:text-base">{{ $option->option_text }}</p>
                                    ...
                                    <div class="flex items-center gap-2">
                                        @if ($isOpen)
                                            <form method="POST" action="{{ route('polls.vote', $poll) }}" class="w-full sm:w-auto">
                                                ...
                                                <button type="submit"
                                                        class="inline-flex w-full items-center justify-center rounded-lg px-3 py-2 text-xs font-semibold sm:w-auto {{ $isSelected ? 'border border-blue-300 text-blue-700 hover:bg-blue-100' : 'bg-blue-600 text-white hover:bg-blue-700' }} btn btn-ghost btn-xs">
                                                    {{ $isSelected ? 'Selected' : 'Vote' }}
                                                </button>
                                            </form>
                                        @else
                                            <span class="inline-flex rounded-lg border border-base-300 px-3 py-2 text-xs font-semibold text-base-content/70">
                                                Voting Closed
                                            </span>
                                        @endif
                                    </div>
