...
class UserThreeCitizenServicesSeeder extends Seeder
{
    public function run(): void
    {
        $user = User::query()->with('resident')->find(3);

        if (! $user || ! $user->resident_id) {
            $this->command?->warn('User ID 3 or its linked resident was not found; service request and complaint demo data were not seeded.');
        ...
        $serviceRequests = [
            ['reference_number' => 'CSR-USER3-001', 'service_type' => 'certificate', 'service_name' => 'Barangay Clearance', 'status' => 'submitted', 'current_step' => 'Application received', 'days_ago' => 2, 'expected_days' => 5, 'notes' => 'For local employment requirements.'],
            ['reference_number' => 'CSR-USER3-002', 'service_type' => 'permit', 'service_name' => 'Business Permit Renewal', 'status' => 'processing', 'current_step' => 'Assessment and fee validation', 'days_ago' => 7, 'expected_days' => 3, 'notes' => 'Renewal documents are under review.'],
            ['reference_number' => 'CSR-USER3-003', 'service_type' => 'assistance', 'service_name' => 'Medical Assistance', 'status' => 'for-release', 'current_step' => 'Approved for release', 'days_ago' => 12, 'expected_days' => 1, 'notes' => 'Bring a valid resident ID when claiming.'],
            ['reference_number' => 'CSR-USER3-004', 'service_type' => 'certificate', 'service_name' => 'Residency Certificate', 'status' => 'completed', 'current_step' => 'Certificate released', 'days_ago' => 20, 'expected_days' => -14, 'notes' => 'Digital copy released to the resident.'],
        ];
        ...
        foreach ($serviceRequests as $row) {
            ...
            CitizenServiceRequest::query()->updateOrCreate(
                ...
                [
                    ...
                    'notes' => $row['notes'],
                    'metadata' => ['channel' => 'demo-seeder', 'user_id' => 3],
                ]
            );
        }
